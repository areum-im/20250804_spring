...
public class UploadController {
  ...
  @PostMapping("/removeFile") // removeFile
  public ResponseEntity<Boolean> removeFile(String fileName) {
    log.info(">>>" + fileName);
    String srcFileName = null;
    try {
      srcFileName = URLDecoder.decode(fileName, "UTF-8"); // 디코딩하는 이유 : 정확한 소스파일명을
      // File.separator(\) :: 확장자
      File file = new File(uploadPath + File.separator + srcFileName); // 해당되는 원본 파일선택
      boolean result = file.delete(); // 첫번째 원본 파일 지움
      File thumbnail = new File(file.getParent(), "s_"+ file.getName()); // 썸네일 파일 선택
      result = thumbnail.delete() && result; // 두번째 썸네일 파일 지움
