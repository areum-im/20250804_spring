...
public class SecurityConfig {
  ...
  // permitAll 할 내용을 복수 개 설정하기 위한 문자 배열 선언
  ...
  protected SecurityFilterChain config(HttpSecurity httpSecurity) throws Exception {
    ...
    httpSecurity.authorizeHttpRequests(new Customizer<AuthorizeHttpRequestsConfigurer<org.springframework.security.config.annotation.web.builders.HttpSecurity>.AuthorizationManagerRequestMatcherRegistry>() {
      ...
      public void customize(AuthorizeHttpRequestsConfigurer<HttpSecurity>.AuthorizationManagerRequestMatcherRegistry auth) {
        ...
        // 인증(Authentication) =======================
        // anyRequest(): 어떠한 요청에도 (이후에 requestMatchers 개별로 지정 불가)
        // auth.anyRequest().permitAll();    // permitAll() : 인증 모두 수용
        //auth.anyRequest().authenticated(); // authenticated() : 모두 인증

        // requestMatchers("url"): 해당 url에 대해서
        ...
        auth.requestMatchers("/sample/**").authenticated(); // 주소를 필터링해서 허용
        // ==========================



        // hasRole(): 권한 여부에 대해서 ================================
        // auth.requestMatchers("/sample/manager").hasRole("MANAGER"); // 권한이 단수일 때
        auth.requestMatchers("/sample/all").hasRole("USER");
        auth.requestMatchers("/sample/manager").access( // 권한이 복수일 때
    ...
    /*httpSecurity.formLogin(new Customizer<FormLoginConfigurer<HttpSecurity>>() {
      ...
      public void customize(FormLoginConfigurer<HttpSecurity> httpSecurityFormLoginConfigurer) {
        httpSecurityFormLoginConfigurer
            ...
            .loginProcessingUrl("/sample/loginSuccess") // 로그인 후 이동할 페이지 직접 설정
